...
- **Media en listados**: chip **Media** debajo de la meta de disponibilidad/fecha en `ListingCard` → un único `RequestMediaModalHost` en `App` (no un `IonModal` por card). Sin adjuntos, no se muestra el chip (`utils/requestMedia.ts`).
