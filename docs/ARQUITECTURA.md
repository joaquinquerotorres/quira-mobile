...
- **Componentes compartidos** (`src/components/listing/`): `ListingCard` compone `CategoryBadge` (píldora+icono), `StatusBadge`, `EstimatePriceBlock` (`range` | `text` | `ownBid`) y `ListingCardFooter`. Tokens de estado en `utils/listingStatus.ts` (incluye `available` / `sent` para detalle pro); estilos de categoría en `utils/categoryStyles.ts`. `FilterChipRow` solo en Mis solicitudes (Mercado/Gestión filtran por modal), con fade de scroll cuando desborda. Alias: `ListingHeroHeader`→`MainHeader` (cabecera compacta), `SegmentedTabBar`→`SegmentTab`, `SearchWithFilter`→`SearchText`. Bottom nav: `IonTabs` en `App.tsx` — cliente con FAB naranja **"Pedir"**; pro sin FAB (**Mercado → Gestión → Calendario → Perfil**; home pro = `/market`). Sin profesional asignado, el footer de la card no muestra copy pasivo.
- **Media en listados**: chip **Media** debajo de la meta de disponibilidad/fecha en `ListingCard` → un único `RequestMediaModalHost` en `App` (no un `IonModal` por card). El contador y el carrusel incluyen **media principal + extras** (`photoUrl`/`videoUrl`/`audioUrl` y `extraPhotoUrls`/`extraVideoUrls`/`extraAudioUrls` vía `collectRequestMedia`). Sin adjuntos, no se muestra el chip (`utils/requestMedia.ts`).
...
- Sin thumbnail: chip **Media** debajo de la fila de disponibilidad/fecha (no en pills ni en footer). Sin profesional asignado, el footer no muestra texto pasivo (el cliente espera pujas en silencio).
...
- Paso 2: Diagnóstico Quira, título, **nivel de riesgo (`risk_level` → LOW / MEDIUM / HIGH)**, **rango de precio estimado por Quira en la zona** (solo lectura; se muestra como «Rango estimado en tu zona (Quira)») y **disponibilidad preferida para realizar el trabajo (sin fecha exacta)**. El nivel de riesgo se muestra como etiqueta no editable en el step 2 y se envía en la creación de la request para rellenar el campo `riskLevel` del backend. **Publicar** envía `estimatedPriceMin`, `estimatedPriceMax` y `aiDiagnosis` (objeto con `min`/`max` en céntimos); **no** se envía `priceAmount`. **Añadir más detalles (opcional)**: fotos, vídeos o audios adicionales (hasta un máximo configurable); texto de ayuda: "Cuanto más detallada sea tu solicitud... más fácil será que los profesionales te hagan una buena oferta". Para audio se puede grabar in situ o elegir desde galería.
