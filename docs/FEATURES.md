...
- Solicitudes: rango de precio IA (`estimatedPriceMin` / `estimatedPriceMax`); contrato de **`POST /requests`** en **[API.md](./API.md)** y vistas en **[ARQUITECTURA.md](./ARQUITECTURA.md)** (NewRequest, listados, detalle). En **listados** el chip **Media** va en las pills de la card (no en el footer). Análisis: ticket Supabase → `/predict` por URL (ver **[BACKEND_PREDICT_UPLOAD.md](./BACKEND_PREDICT_UPLOAD.md)**). Para **publicar** hace falta **teléfono de cliente** verificado; la app avisa en el paso 1 y bloquea el análisis si aún no aplica.
