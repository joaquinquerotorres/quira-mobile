...
- Solicitudes: rango de precio IA (`estimatedPriceMin` / `estimatedPriceMax`); contrato de **`POST /requests`** en **[API.md](./API.md)** y vistas en **[ARQUITECTURA.md](./ARQUITECTURA.md)** (NewRequest, listados, detalle). En **listados** el chip **Media** va debajo de la disponibilidad/fecha. Pendientes sin pro: **«Esperando propuestas»**. Análisis: ticket Supabase → `/predict` por URL (ver **[BACKEND_PREDICT_UPLOAD.md](./BACKEND_PREDICT_UPLOAD.md)**). Para **publicar** hace falta **teléfono de cliente** verificado.
