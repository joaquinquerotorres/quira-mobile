...
describe('getVideoUploadConnectionHint', () => {
  ...
  it('en web sin connection devuelve unknown', async () => {
    expect(await getVideoUploadConnectionHint()).toBe('unknown');
  });

  it('en web con effectiveType 3g devuelve slow_or_unreliable', async () => {
    Object.defineProperty(navigator, 'connection', {
      configurable: true,
  ...
  it('en web con 4g devuelve unknown (no se distingue Wi‑Fi vs datos móviles)', async () => {
    Object.defineProperty(navigator, 'connection', {
      configurable: true,
